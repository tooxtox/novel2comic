...
def segment_novel():
    ...
    prompt = f"""将以下小说内容转换成漫画分镜，每页{segments_per_page}个分镜。

小说内容：
{text[:5000]}

只返回JSON，格式如下：
{{"pages": [{{"page_number": 1, "segments": [{{"segment_number": 1, "scene_description": "场景描述", "dialogue": "对话", "shot_type": "特写", "style_prompt": "black and white manga style"}}]}}]}}"""
    ...
    try:
        ...
        payload = {
            ...
            'messages': [
                {'role': 'system', 'content': '你是一个专业的漫画分镜师，擅长将小说转换为漫画分镜脚本。'},
        ...
        try:
            ...
            if json_match:
                ...
                return jsonify({'success': True, 'data': parsed})
            return jsonify({'error': '无法解析LLM返回的JSON', 'raw': content}), 500
