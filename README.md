...
**推荐配置**:
- 文本 LLM: 火山引擎 `doubao-pro-32k` 或 `deepseek-chat`
- 图像生成: 火山引擎 `doubao-seedream-3-0-t2i-250415`
...
很多图像生成 API 在生成中文对话气泡时会出现文字乱码。本项目内置**自动检测 + 修复**机制:

### 工作流程

```
生成图片 → LLM 视觉检测文字 → 发现乱码?
                         ↓ 是
                    LLM 分析原因
                         ↓
                  生成修复 prompt
...
- **检测阶段**: 调用视觉 LLM(如 `minimax-vl`)分析图片中的文字是否乱码
- **分析阶段**: LLM 返回乱码原因(模型不支持中文/字体渲染错误等)和修复策略
- **重生成阶段**: 用修复后的 prompt 重新调用图像 API,最多重试 1 次

### 触发条件

- **分镜图**: 仅在分镜有对白(`dialogue` 字段非空)时触发检测
- **整页图**: 始终检测(因为包含多段对话)
- **角色人设图**: 不检测(通常不含文字)
...
打开浏览器 F12 控制台,可查看:
```
[GarbleCheck] 分析结果: {has_garble: true, garble_level: "partial", ...}
[FixGarble] Step 1: Analyzing garble cause...
[FixGarble] Step 2: Regenerating image with fixed prompt...
```
...
| `/api/check-text-garble` | POST | 检测图片文字是否乱码 |
| `/api/fix-text-garble` | POST | 分析乱码并重新生成图片 |
