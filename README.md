...
| **对话气泡** | 自动为分镜添加对话气泡,支持 8 种类型(对话/心理/喊叫/耳语/爆发/旁白/方框/拟声词) |
| **气泡位置编辑** | 拖拽式气泡位置编辑器,带类型可视化预览,可逐气泡切换 dialogue_type 后保存 |
| **气泡渲染方式** | 支持"本地 PIL 叠加"(中文零乱码,默认)或"由图像 API 端到端绘制"(跳过 PIL,速度更快) |
...
- `/` 是宣传页(公开,无需登录),展示核心功能与示例作品
- 点宣传页的 **开始使用** 进入 `/app`,未登录时是登录/注册页,已登录直接进工作台
...
- **批量整页生成** — 一键生成全部整页漫画(支持暂停/继续,WebSocket 实时进度)
  - 批量生成时连续传递上一页图作为参考

### 第六步:查看漫画预览

1. 进入 **漫画预览** Tab
2. 查看已生成的整页漫画,点击图片可放大预览、编辑气泡位置
...
- **参考强度** — 0–1,控制角色参考图/上一页参考图的影响力度
  ...
  - **0.0–0.5** — 弱参考(允许变化)
- **越界气泡** — 对话气泡是否可超出分镜框
- **气泡渲染方式** — 选择图像生成 API 是否自己画气泡
  - **本地 PIL 叠加** (默认) — API 只产干净画面, 后端 PIL 绘制气泡与中文 (零乱码); 支持拖拽编辑气泡位置
  - **由图像 API 绘制** — API 端到端画气泡 (按 dialogue_type 形状), 跳过 PIL; 速度更快但气泡位置由模型决定,不可编辑
...
[API 模式 — 由图像 API 端到端绘制气泡]
  分镜 prompt 末尾追加每 panel 的 dialogue_type → 形状映射 + 对白内容
  → 图像 API 在画面内绘制对应形状的气泡
  → 后端跳过 PIL 叠加, 直接返回 API 产出的图像
```
...
| 设计点 | 说明 |
|--------|------|
| **API 不渲染文字 (PIL 模式)** | 图像生成 prompt 末尾强制追加 "NO text, NO speech bubble, NO caption, NO dialogue" |
| **后端自动叠加 (PIL 模式)** | PIL 叠加在 `/api/generate-page` 内部完成,前端无感知 |
| **API 端到端渲染 (API 模式)** | 由图像 API 在画面内按 dialogue_type 形状绘制气泡,后端跳过 PIL |
| **两种模式可切换** | 高级设置 → 气泡渲染方式, 可在 PIL/API 之间切换, 配置持久化到 config.json + localStorage |
...
| **编辑器内联切换类型** | 气泡编辑器内每个气泡右上角有类型下拉, 切换即时按 `[data-type]` 切换形状样式 |
| **文字自适应** | fit_text_to_box 自动折行+缩字号,长对话也能塞进气泡 |
| **整页网格对齐** | PIL 叠加时使用与 prompt 相同的 rows×cols 网格布局计算 panel 位置 |

### 气泡位置编辑器 (PIL 模式可用)

打开任一已生成的整页, 点击「编辑气泡位置」, 弹出干净的图片画布 + 可拖拽气泡覆盖层:

- 每个气泡显示成接近最终成图的样式 (按 `dialogue_type` 应用对应形状/颜色/边框)
- 右上角带可点击的类型下拉, 切换 `dialogue_type` 后形状立即换, 保存时写入新类型
- 鼠标/触摸拖动气泡改变位置, 像素坐标自动换算到原图坐标后保存
- 「保存」调用 `/api/reposition-bubbles`, 在干净图上重画并覆盖输出图, 调整不会改动 API 原始画面

**API 模式下不显示该按钮** —— 气泡位置由模型决定, 没有可用坐标.
...
历史作品的整页图若是旧架构(图像 API 直渲文字)产物,仍会保留原乱码文字。要获得干净气泡,在**分镜管理** Tab 中对相应页面点击「整页生成」重新生成即可,新图片会自动走 PIL 叠加流程。

### 何时选 PIL / 何时选 API

| 场景 | 推荐模式 |
|------|---------|
| **中文/日文等非拉丁语言** | PIL (默认) — API 直渲中文几乎必乱码 |
| **需要拖拽调整气泡位置** | PIL — API 模式气泡位置由模型决定,没有可用坐标 |
| **追求英文/纯拉丁语漫画 + 速度** | API — 一次出图,跳过 PIL,手绘字体观感更好 |
| **模型已针对漫画对白做了 SFT** | API — 模型对 speech bubble 形态理解深 |
| **需要 8 种 dialogue_type 形状精确控制** | PIL — 8 种气泡本地绘制,精确可控 |
...
| 页面 / 接口 | 方法 | 说明 |
|------|------|------|
| `/` | GET | 宣传页(公开),CTA 进入 `/app` |
| `/app` | GET | 未登录显示登录页,已登录显示工作台 |
...
| `/api/generate-page` | POST | 一页完整漫画生成(支持上一页参考图、角色参考图、`bubble_render_mode` 切换 PIL/API 模式) |
| `/api/comic-to-novel` | POST | 漫画图片 → 小说文本 |
| `/api/reposition-bubbles` | POST | 在干净图上按用户坐标重新烘气泡 (支持调整 dialogue_type), 覆盖输出图 |
...
### Q: 生成的图片对话文字会乱码吗?
A: 不会。本项目采用 PIL 本地渲染架构:图像生成 API 只产出无文字的干净画面,对话文字+气泡由后端 PIL 使用系统中文字体(微软雅黑/黑体/宋体)直接绘制,彻底杜绝图像 API 渲染中文乱码的问题。若历史作品图片仍是旧架构(图像 API 直渲文字)产物,请在分镜管理中重新「整页生成」即可获得干净气泡。
...
- [x] 8 种对话气泡类型(dialogue/thought/shout/whisper/burst/narration/box/caption)
- [x] 气泡位置拖拽编辑器(按 dialogue_type 可视化预览 + 右上角类型下拉切换)
- [x] 气泡渲染方式全局开关(本地 PIL 叠加 / 由图像 API 端到端绘制)
