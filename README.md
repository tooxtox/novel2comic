...
| **乱码按页修复** | 整页为单位检测+修复,直接用分镜原文调用图像 API 重新生成,无需 LLM 二次分析 |
...
- **参考强度** — 0–1,控制角色参考图/上一页参考图的影响力度
  - **0.8–1.0** — 高度一致(严格保持角色外观,修复乱码时推荐 0.85)
  - **0.5–0.8** — 中度一致(推荐)
  - **0.0–0.5** — 弱参考(允许变化)
- **越界气泡** — 对话气泡是否可超出分镜框
...
很多图像生成 API 在生成中文对话气泡时会出现文字乱码或与原文不符。本项目在 **「文字乱码清理」** Tab 提供**按页检测 + 按页修复**的一体化工具,无需先检测也能直接修复。
...
[修复阶段 — 不调 LLM, 直接用原文 + 原图参考]
  原图(base64) + 分镜原文(prompt) → 图像 API
  → reference_strength=0.85 保持构图
  → 只重画文字部分
  → 生成预览图 → 用户确认 → 覆盖原图
```
...
| 设计点 | 说明 |
|--------|------|
| **按页聚合** | 把同一页所有勾选分镜合并成一次调用,大幅减少 API 调用次数 |
| **原图参考** | 修复时把整页原图转 base64 传给图像 API,作为 `image` 参考图 |
| **不调 LLM** | 修复阶段跳过 LLM 分析,直接用分镜原文拼接 prompt 重新生成 |
| **双向检测** | LLM 检测同时识别"文字乱码"和"与原文不一致"两种问题 |
| **预览确认** | 修复后显示原图 vs 新图对比,用户确认后才覆盖原图 |
| **并发控制** | 修复并发 2,检测并发 2 (整页图较大,避免 API 限流) |

### 修复 prompt 策略

修复 prompt 采用"OCR 重写任务"模式,包含:

1. **任务定义** — 明确告知模型这是 text-redrawing task,只重画文字
2. **对话列表** — 按分镜编号列出每个气泡的原文:
   ```
   [Panel 1]
     Scene: 主角在咖啡馆
     Bubble 1: "你好"
     Bubble 2: "最近怎么样"
   ```
3. **8 条严格文字渲染规则** — 禁止乱码/方框/问号/ASCII/混用语言
4. **4 条艺术规则** — 禁止改动构图/角色/画风
5. **4 种 CRITICAL FAILURE** — 让模型知道哪些是严重错误

### 使用步骤

1. 进入 **「文字乱码清理」** Tab
2. 选择要处理的页面(可「全选 / 清空选择 / 仅选有图」)
3. **可选**: 点击「🔍 一键检测乱码」让 LLM 视觉分析每页(传整页图 + 该页所有分镜原文)
4. 点击「🔧 按页修复整页图」直接修复(无需先检测)
5. 在右侧预览对比原图 vs 新图
6. 点击「✅ 使用修复版」覆盖原图,或「❌ 保留原图」放弃修复
...
| `/api/check-text-garble` | POST | 检测整页图片文字是否乱码/与原文不符(传 `original_text` 双向检测) |
| `/api/fix-text-garble` | POST | 用分镜原文+原图参考重新生成(不调 LLM,`reference_strength=0.85`) |
| `/api/accept-garble-fix` | POST | 确认接受修复版,覆盖原图 |
...
### Q: 生成的图片文字乱码怎么办?
A: 进入 **「文字乱码清理」** Tab,勾选有乱码的页面分镜,点击「🔧 按页修复整页图」。系统会用原图作参考 + 分镜原文作 prompt 重新生成,保持构图只改文字。如果仍有问题:
- 检查分镜原文是否准确(可在分镜管理中编辑)
- 调整参考强度(高级设置中,建议 0.8-0.9)
- 先用「🔍 一键检测乱码」让 LLM 视觉分析确认问题
...
- [x] 文字乱码按页检测+修复(不调 LLM,直接用原文+原图参考)
